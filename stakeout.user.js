...
async function fetchFactionMembersWithCache(factionID) {
  ...
  if (cachedItem && isCacheValid(cachedItem, dynamicCacheDurationMs)) {
    console.log(
      `[Stakeout] Using cached faction data for ${factionID} (cache valid for ${
        dynamicCacheDurationMs / 1000
      }s based on ${currentMonitoringIntervalSecs}s interval)`
    );
...
function createMemberElement(member, categoryName) {
  ...
  if (ffsEstimateDisplay || yataDisplay || bspTBSDisplay) {
    ...
    memberDiv.appendChild(spySummaryRow);
  }
  ...
  // YATA > FFScouter > BSP for the main spy icon
  const spyData =
  ...
  memberDiv.appendChild(detailsAndActionsWrapper);
...
function observe() {
  ...
  window.StakeOutInterval = setInterval(() => {
    ...
    if (
      (profileStatusElement && document.getElementById("stakeoutCheckbox")) ||
      (factionProfileElement && document.getElementById(FACTION_CONTROLS_CONTAINER_ID))
    )
