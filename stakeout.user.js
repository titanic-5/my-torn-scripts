...
// @connect      stakeout.titanic-5.uk
// @connect      *
// ==/UserScript==

const DIBS_ENDPOINT_URL = "https://stakeout.titanic-5.uk/dibs";
const DIBS_PRESS_DURATION_MS = 500;
...
const BSP_API_ID = "stakeout-bsp-api-key-input";
...
const BSP_KEY = "stakeoutBSPApiKey";
const TIMER_KEY = "stakeoutFactionMonitoringIntervalSecs";
const BSP_BASE_URL = "http://www.lol-manager.com/api/battlestats/[APIKEY]/[TARGET_USER_ID]/TitanicStakeout";
...
const BSP_SPIES_STORE_NAME = "bspFactionSpies";
...
function getActivePlayerInfo() {
    ...
    if (userLink) {
        ...
        const urlParams = new URLSearchParams(userLink.search);
...
function startDibsPolling() {
    ...
    dibsSyncIntervalId = setInterval(async () => {
        if (!isApiKeySet()) return;
        const success = await fetchDibsData();
        if (success) updateDibsStaticUI();
    }, 3000);

    dibsAnimationIntervalId = setInterval(updateDibsAnimations, 50);
}

function stopDibsPolling() {
    if (dibsSyncIntervalId) clearInterval(dibsSyncIntervalId);
    if (dibsAnimationIntervalId) clearInterval(dibsAnimationIntervalId);
}

function updateDibsStaticUI() {
    if (!currentStatuses) return;
    currentStatuses.forEach(member => {
        const userId = member.userID;
        const dibsInfo = allDibs[userId];
        const textSpan = document.getElementById(`dibs-text-${userId}`);
        const attackBtn = document.getElementById(`attack-btn-${userId}`);
        const bgDiv = document.getElementById(`dibs-bg-${userId}`);

        if (!textSpan || !attackBtn) return;

        if (dibsInfo) {
            const isMyDibs = dibsInfo.dibsById === activePlayer.id;
            textSpan.style.display = 'block';
            textSpan.textContent = `Dibbed by\n${dibsInfo.dibsByName}`;

            if (!isMyDibs) {
                attackBtn.style.backgroundColor = '#555';
                attackBtn.style.cursor = 'not-allowed';
                attackBtn.style.pointerEvents = 'none';
                attackBtn.href = "#";
                attackBtn.target = "";
                attackBtn.title = `Dibbed by ${dibsInfo.dibsByName}`;
                attackBtn.onmouseover = () => (attackBtn.style.backgroundColor = '#555');
                attackBtn.onmouseout = () => (attackBtn.style.backgroundColor = '#555');
            } else {
                ...
                attackBtn.onmouseout = () => (attackBtn.style.backgroundColor = '#c0392b');
            }
        } else {
            ...
            if (bgDiv) bgDiv.style.background = '#383838';
        }
    });
}

function updateDibsAnimations() {
    const now = Date.now();
    ...
    currentStatuses.forEach(member => {
        ...
        if (!bgDiv || !dibsInfo) return;

        const remaining = dibsInfo.clientExpires - now;
        if (remaining <= 0) {
            bgDiv.style.background = '#383838';
            return;
        }

        const isMyDibs = dibsInfo.dibsById === activePlayer.id;
        const dibsColor = isMyDibs ? 'rgba(46, 204, 113, 0.6)' : 'rgba(231, 76, 60, 0.6)';
        const percentage = Math.max(0, (remaining / totalDuration) * 100);

        bgDiv.style.background = `linear-gradient(to left, ${dibsColor} ${percentage}%, #383838 ${percentage}%)`;
    });
}

async function fetchDibsData() {
    return new Promise(resolve => {
        GM_xmlhttpRequest({
            method: "GET",
            url: DIBS_ENDPOINT_URL,
            timeout: 5000,
            onload: (response) => {
                try {
                    ...
                    for (const targetId in data) {
                        allDibs[targetId] = {
                            ...data[targetId],
                            clientExpires: clientNow + (data[targetId].remaining || Math.max(0, data[targetId].expires - clientNow))
                        };
                    }
                    resolve(true);
                } catch (e) {
                    console.error("[Stakeout] Error parsing dibs data:", e);
                    allDibs = {};
                    resolve(false);
                }
            },
            onerror: (err) => {
                console.error("[Stakeout] Network error fetching dibs data.", err);
                resolve(false);
            },
            ontimeout: () => {
                console.error("[Stakeout] Request timeout fetching dibs data.");
                resolve(false);
            }
        });
    });
...
async function claimDibs(targetId) {
    if (!activePlayer.id) {
        alert("Could not identify you. Cannot claim dibs.");
        return;
    }
    ...
    GM_xmlhttpRequest({
        ...
        data: JSON.stringify({
            ...
            dibsById: activePlayer.id,
            dibsByName: activePlayer.name
        }),
        ...
        onerror: (err) => console.error("[Stakeout Dibs] Error claiming dibs:", err)
    });
}

async function releaseDibs(targetId) {
    if (!activePlayer.id) return;

    GM_xmlhttpRequest({
        method: "DELETE",
        url: `${DIBS_ENDPOINT_URL}/${targetId}`,
        headers: { "Content-Type": "application/json" },
        data: JSON.stringify({ dibsById: activePlayer.id }),
        onload: async () => {
            await fetchDibsData();
            updateDibsStaticUI();
        }
...
function openDB() {
    ...
    dbPromise = new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains(BSP_SPIES_STORE_NAME)) db.createObjectStore(BSP_SPIES_STORE_NAME, { keyPath: "factionID" });
...
async function clearOldSpyDataFromDB() {
    for (const storeName of [YATA_SPIES_STORE_NAME, FFSCOUTER_SPIES_STORE_NAME, BSP_SPIES_STORE_NAME]) {
...
async function fetchApi(endpoint, selections = "basic", apiKey = currentApiKey) {
    return new Promise((resolve) => {
        ...
        const isBSPApi = endpoint.includes("lol-manager.com/api/battlestats");

        if (isTornApi && (!apiKey || apiKey === API_KEY_PLACEHOLDER)) return resolve({ error: { error: "API Key not set" } });
        if (!isTornApi && !isFFScouterApi && !isBSPApi && !apiKey && !endpoint.includes("key="))
            return resolve({ error: { error: "API Key not set for external service" } });
        ...
        GM_xmlhttpRequest({
            ...
            onload: (response) => {
                try {
                    const data = JSON.parse(response.responseText);
                    if (response.status < 200 || response.status >= 300 || data?.error?.error || (data?.Result === 0 && isBSPApi)) {
                        const errorMsg = data?.error?.error || data?.Reason || `HTTP Error ${response.status}`;
...
async function fetchYataSpies(factionID) {
    ...
    if (freshData?.error) {
        ...
        if (code === 2 && (msg.includes("No spies") || msg.includes("faction not found")))
            console.log("[Stakeout] YATA: No spies for faction or faction not found:", factionID);
        else {
...
async function fetchBSPSpyData(factionID, memberIDs) {
    const bspApiKey = localStorage.getItem(BSP_KEY);
    ...
    async function throttleFetch(items, maxConcurrent, fetchFn) {
        ...
        for (const item of items) {
            const promise = fetchFn(item).finally(() => pool.delete(promise));
            pool.add(promise);
    ...
    const fetchSingleBSP = async (target) => {
        const data = await fetchApi(target.url);
        ...
        if (data && data.Result === 1) {
            ...
            if (data.error.error?.includes("API Key") || data.error.error?.includes("Access Denied")) {
                createApiErrorDisplay("BSP", "Invalid API Key or Access Denied", "bsp-api-error-message");
...
function createMemberElement(member, categoryName) {
    ...
    const memberDiv = createStyledElement("div", {
        ...
        borderLeft: `3px solid ${member.status === "Okay"
            ? "#4CAF50"
    ...
    const contentContainer = createStyledElement('div', {
        position: 'relative',
        zIndex: 2,
        width: '100%',
        padding: '6px 8px',
    ...
    const handlePressStart = (e) => {
        ...
        if (dibsPressTimer) clearTimeout(dibsPressTimer);
        dibsPressTimer = setTimeout(() => {
            claimDibs(member.userID);
            dibsPressTimer = null;
        }, DIBS_PRESS_DURATION_MS);
    };
    ...
    memberDiv.addEventListener("mousedown", handlePressStart);
    memberDiv.addEventListener("mouseup", handlePressEnd);
    memberDiv.addEventListener("mouseleave", handlePressEnd);
    memberDiv.addEventListener("touchstart", handlePressStart, { passive: false });
    memberDiv.addEventListener("touchend", handlePressEnd);
    memberDiv.addEventListener("touchcancel", handlePressEnd);

    const backgroundDiv = createStyledElement('div', {
        position: 'absolute',
        top: 0, left: 0, right: 0, bottom: 0,
        zIndex: 1,
        backgroundColor: '#383838',
    }, { id: `dibs-bg-${member.userID}` });
    memberDiv.appendChild(backgroundDiv);

    const dibsText = createStyledElement("span", {
        position: 'absolute',
        top: '50%', left: '50%',
        ...
        whiteSpace: 'pre-line',
        display: 'none'
    }, { id: `dibs-text-${member.userID}` });
    memberDiv.appendChild(dibsText);
    ...
    const bspTBSValue = sBSP?.tbs > 0 ? sBSP.tbs : null;
    ...
    const bspTBSDisplay = bspTBSValue ? formatStatValue(bspTBSValue) : null;

    if (ffsEstimateDisplay || yataDisplay || bspTBSDisplay) {
        ...
        if (ffsEstimateDisplay) {
            ...
            const ffsSpan = createStyledElement(
                "span",
                {
                    ...coloredSpanStyles,
        ...
        if (yataDisplay) {
            const yataSpan = createStyledElement(
                "span",
                {
                    ...plainSpanStyles,
                    color: "#76D7C4",
        ...
        if (bspTBSDisplay) {
            const bspSpan = createStyledElement(
                "span",
                {
                    ...plainSpanStyles,
                    color: "#F39C12",
                },
                { textContent: `BSP: ${bspTBSDisplay}` }
            );
            contentParts.push(bspSpan.outerHTML);
        }
    ...
    const nameContainer = createStyledElement("div", {
        display: "flex",
        alignItems: "center",
        marginBottom: "1px",
        maxWidth: "130px",
    ...
    nameContainer.append(
        onlineStatusIcon,
        createStyledElement("span", { fontWeight: "bold", color: "#E0E0E0" }, { textContent: member.name, title: member.name })
    );
    ...
    const spyData =
        member.yataSpyData && (member.yataSpyData.total > 0 || Object.values(member.yataSpyData).some((v) => typeof v === "number" && v > 0 && v !== -1))
            ...
            : member.ffscouterSpyData && (member.ffscouterSpyData.bs_estimate !== null || member.ffscouterSpyData.fair_fight !== null)
                ...
                : member.bspSpyData?.tbs > 0
                    ? { source: "bsp", data: member.bspSpyData }
                    : null;

    if (spyData) {
        const baseColor = spyData.source === "yata" ? "#3498DB" : spyData.source === "ffscouter" ? "#555" : "#E67E22";
        const hoverColor = spyData.source === "yata" ? "#2980B9" : spyData.source === "ffscouter" ? "#444" : "#D35400";
        const borderColor = spyData.source === "yata" ? "#2980B9" : spyData.source === "ffscouter" ? "#5a6268" : "#A04000";

        const spyButton = createActionButton("🕵️‍♂️", "", "#", baseColor, hoverColor, {
        ...
        spyButton.onmouseover = (event) => {
                ...
                innerHTML = `
                    <div style="font-family: Verdana, Arial, sans-serif;">
                        ...
                        <div style="font-size: 0.85em; color: #999; text-align: right; margin-top: 5px;">${formatSpyTimestamp(
                            s.update ||
                            Math.max(
                                s.strength_timestamp || 0,
                                s.speed_timestamp || 0,
                                s.defense_timestamp || 0,
                                s.dexterity_timestamp || 0,
                                s.total_timestamp || 0
                            )
                        )}</div>
                    </div>`;
            } else if (spyData.source === "ffscouter") {
                innerHTML = `
                    <div style="font-family: Verdana, Arial, sans-serif;">
                        ...
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 5px 12px; margin-bottom: 10px; font-size: 0.95em;">
                            <span style="font-weight: bold; color: #ccc;">Fair Fight:</span> <span style="color: #f0f0f0; text-align: right;">${s.fair_fight !== null ? s.fair_fight.toFixed(2) : "N/A"
                    }</span>
                            <span style="font-weight: bold; color: #ccc;">BS Estimate:</span> <span style="color: #f0f0f0; text-align: right;">${s.bs_estimate_human || (s.bs_estimate ? formatStatValue(s.bs_estimate) : "N/A")
                    }</span>
                        </div>
            ...
            } else if (spyData.source === "bsp") {
                innerHTML = `
                    <div style="font-family: Verdana, Arial, sans-serif;">
                        <div style="font-size: 1.15em; font-weight: bold; color: #F39C12; margin-bottom: 10px; padding-bottom: 6px; border-bottom: 1px solid #4a4a4a; text-align: center;">BSP Report</div>
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 5px 12px; margin-bottom: 10px; font-size: 0.95em;">
                            <span style="font-weight: bold; color: #ccc;">Total Battle Stats:</span> <span style="color: #f0f0f0; text-align: right;">${s.tbs ? formatStatValue(s.tbs) : "N/A"
                    }</span>
                            <span style="font-weight: bold; color: #ccc;">Prediction Date:</span> <span style="color: #f0f0f0; text-align: right;">${s.predictionDate || "N/A"
                    }</span>
                        </div>
                        <div style="font-size: 0.85em; color: #999; text-align: right; margin-top: 10px; padding-top: 5px; border-top: 1px solid #4a4a4a;">Data age: ${formatSpyTimestamp(
                        s.last_updated
                    )}</div>
                    </div>`;
            }
            ...
            tooltip.style.top = `${rect.bottom +
                window.scrollY +
                7 +
                (rect.bottom + tooltip.offsetHeight > window.innerHeight - 10 ? -(tooltip.offsetHeight + rect.height + 14) : 0)
                }px`;
        };
    ...
    const profileButton = createActionButton("👤", "View Profile", `https://www.torn.com/profiles.php?XID=${member.userID}`, "#555", "#666", null, tooltipBug);
    ...
    attackButton.id = `attack-btn-${member.userID}`;
    actionsContainer.append(profileButton, attackButton);

    const detailsAndActionsWrapper = createStyledElement("div", {
        display: "flex",
    ...
    detailsAndActionsWrapper.append(memberInfoContainer, actionsContainer);
    contentContainer.appendChild(detailsAndActionsWrapper);
    memberDiv.appendChild(contentContainer);
...
function createCategoryElement(categoryName, membersInCategory, factionID) {
    ...
    const updateHeaderContent = (isCollapsed) =>
        (categoryHeader.innerHTML = `${categoryName} <span>(${membersInCategory.length}) ${isCollapsed ? "▼" : "▲"}</span>`);
...
function updateFactionDisplay(memberStatusesToDisplay, factionID) {
    ...
    if (!displayContainer) {
        ...
        const controlsContainer = document.getElementById(FACTION_CONTROLS_CONTAINER_ID);
        if (targetElement) {
            if (controlsContainer?.parentNode === targetElement && controlsContainer.nextSibling)
                targetElement.insertBefore(displayContainer, controlsContainer.nextSibling);
...
async function fetchMonitorAndUpdate(factionID, stakeoutCheckbox, isInitialCall = false) {
    ...
    document.getElementById("bsp-api-error-message")?.remove();
    ...
    if (tornFactionData?.error || !tornFactionData?.members) {
        ...
        if (cw)
            cw.innerHTML = `<p style="color: #ff6666; text-align: center; padding: 10px;">Error fetching faction data: ${tornFactionData?.error?.error || "No member data"
                }.</p>`;
        return;
    }
    ...
    currentBSPSpyData = (await fetchBSPSpyData(factionID, memberIDs)) || {};

    const previousStatusesMap = new Map(currentStatuses.map(m => [m.userID, m.status]));
    currentStatuses = Object.entries(tornFactionData.members).map(([_, m]) => ({
        ...
        bspSpyData: currentBSPSpyData[m.id.toString()] || null,
    }));

    currentStatuses.forEach(member => {
        const dibs = allDibs[member.userID];
        if (dibs && dibs.dibsById === activePlayer.id) {
            const wasInHospital = previousStatusesMap.get(member.userID) === 'Hospital';
            const isNowOkay = member.status === 'Okay';

            if (wasInHospital && isNowOkay && !targetsToAutoUndibs.has(member.userID)) {
                targetsToAutoUndibs.set(member.userID, Date.now());
            }
        }

        if (member.status !== 'Okay' && targetsToAutoUndibs.has(member.userID)) {
            targetsToAutoUndibs.delete(member.userID);
        }
    })

    targetsToAutoUndibs.forEach((timestamp, targetId) => {
        if (Date.now() - timestamp > 30 * 1000) {
            console.log(`[Stakeout Dibs] Auto-releasing dibs on ${targetId} after 30s of being Okay.`);
            releaseDibs(targetId);
            targetsToAutoUndibs.delete(targetId);
        }
    });
...
async function saveSpyApiKeys() {
    ...
    await processKey(BSP_API_ID, BSP_KEY, BSP_SPIES_STORE_NAME, "bsp-api-error-message");
...
function addFactionStakeoutElements(factionPageElement, factionID) {
    ...
    const stakeoutCheckbox = createStyledElement(
        "input",
        { marginRight: "5px", cursor: "pointer" },
        { type: "checkbox", id: "factionStakeoutCheckbox" }
    );
    ...
    const toggleDisplayButton = createFactionControlButton(
        localStorage.getItem(COLLAPSED_KEY) === "true" ? "Show Member List" : "Hide Member List",
        () => {
    ...
    const spiesButton = createFactionControlButton("Spies Config", openSpiesModal, null, {
        backgroundColor: "rgb(66 71 207)",
        border: "1px solid rgb(66 71 207)",
    });
...
function addStakeoutElementsToProfiles(statusElement) {
    ...
    stakeoutCheckbox.addEventListener("change", () =>
        stakeoutCheckbox.checked ? ((profileUserWasOkay = false), startStakeout(parseInt(intervalDropdown.value))) : stopStakeout()
    );
    intervalDropdown.addEventListener("change", () => {
...
function observe() {
    ...
    window.StakeOutInterval = setInterval(() => {
        ...
        } else if (url.includes("factions.php?step=profile&ID=")) {
        ...
        if (
            (document.querySelector(PROFILE_SELECTOR) && document.getElementById("stakeoutCheckbox")) ||
            (document.querySelector(FACTION_SELECTOR) && document.getElementById(FACTION_CONTROLS_CONTAINER_ID))
        ) {
