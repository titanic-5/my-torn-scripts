...
function createMemberElement(member, categoryName) {
  ...
  const handlePressEnd = (e) => {
    ...
    if (dibsPressTimer) clearTimeout(dibsPressTimer);
    dibsPressTimer = null;
    if (!e.target.closest("a") && (e.type === "mouseup" || e.type === "mouseleave")) {
      e.preventDefault();
    }
  };
  ...
  memberDiv.addEventListener("touchstart", handlePressStart, { passive: true });
  memberDiv.addEventListener("touchend", handlePressEnd, { passive: true });
  memberDiv.addEventListener("touchcancel", handlePressEnd, { passive: true });
  memberDiv.addEventListener("touchmove", handleTouchMove, { passive: true });

  const backgroundDiv = createStyledElement(
    "div",
    {
      position: "absolute",
      top: 0,
  ...
  const dibsText = createStyledElement(
    "span",
    {
      position: "absolute",
      top: "50%",
      ...
      if (spyData.source === "yata") {
        ...
        innerHTML = `
                    <div style="font-family: Verdana, Arial, sans-serif;">
                        ...
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 5px 12px; margin-bottom: 10px; font-size: 0.95em;">
                            <span style="font-weight: bold; color: #ccc;">Total Battle Stats:</span> <span style="color: #f0f0f0; text-align: right;">${
                              s.tbs ? formatStatValue(s.tbs) : "N/A"
                            }</span>
                            <span style="font-weight: bold; color: #ccc;">Prediction Date:</span> <span style="color: #f0f0f0; text-align: right;">${
                              s.predictionDate || "N/A"
                            }</span>
                        </div>
  ...
  const profileButton = createActionButton(
    "👤",
    "View Profile",
    `https://www.torn.com/profiles.php?XID=${member.userID}`,
    "#555",
    "#666",
    null,
    tooltipBug
...
async function fetchMonitorAndUpdate(factionID, stakeoutCheckbox, isInitialCall = false) {
  ...
  if (tornFactionData?.error || !tornFactionData?.members) {
    ...
    if (cw)
      cw.innerHTML = `<p style="color: #ff6666; text-align: center; padding: 10px;">Error fetching faction data: ${
